...
if col2.button("🔄 Làm mới hệ thống"): st.cache_data.clear()

# 2. HÀM TẠO DỮ LIỆU DỰ PHÒNG (FIX LỖI LENGTH)
def get_mock_100():
    return pd.DataFrame({'Mã CK':['FPT','HPG','SSI'], 'Giá':[135,29,36], '+/-':[1,1,1], '%':[1,1,1], 'KL':[10**6]*3})

def get_mock_idx():
    # Tạo 20 dòng dữ liệu cố định để không bao giờ lệch chiều dài
    df = pd.DataFrame({
        'time': pd.date_range(end='2026-05-10', periods=20),
        'close': [1250.0] * 20,
        'volume': [500000000] * 20
    })
    df['MA20'], df['V_MA20'] = 1250.0, 500000000.0
    return df
...
def get_tcbs():
    try:
        ...
        d = requests.get(url, headers={'User-Agent':'Mozilla'}, timeout=5).json()['data']
...
def get_cafef():
    res = []
    try:
        h = requests.get("https://s.cafef.vn/ajax/KhuyenNghi_Update.aspx?PageIndex=1&PageSize=20", headers={"User-Agent":"Mozilla"}).text
...
if err1 or err2: st.error("⚠️ Đang sử dụng dữ liệu dự phòng do kết nối mạng bị chặn.")
